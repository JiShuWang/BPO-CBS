...
class LitClassifier(pl.LightningModule):
    def __init__(self, backbone, learning_rate=1e-3, GradientBoostingRegressor=None, BaggingRegressor=None, RandomForestRegressor=None, KNeighborsRegressor=None):
        super().__init__()
    ...
    def evaluate(self, batch, stage=None):
        ...
        y_hat = torch.cat((y_hat, torch.reshape(torch.from_numpy(self.BaggingRegressor.predict(x)).cuda(), (-1, 1))), dim=1)
        y_hat = torch.cat((y_hat, torch.reshape(torch.from_numpy(self.RandomForestRegressor.predict(x)).cuda(), (-1, 1))), dim=1)
        y_hat = torch.cat((y_hat, torch.reshape(torch.from_numpy(self.KNeighborsRegressor.predict(x)).cuda(), (-1, 1))), dim=1)
...
def cli_main():
    ...
    for train_index, test_index in KF.split(X):
        ...
        MLP = Backbone()
        model = LitClassifier(MLP, args.learning_rate, GradientBoostingRegressor=GradientBoosting, BaggingRegressor=Bagging, RandomForestRegressor=RandomForest,
